...
import (
	"database/sql"
	"html/template" // Mengelola template HTML yang ditampilkan
	"net/http"
	"path/filepath"
)

func NewUpdateEmployeeController(db *sql.DB) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) { // Handler mengupdate data
		if r.Method == "POST" { // Jika user submit, maka program memulai method POST, lalu data diupdate
			...
			return
		} else if r.Method == "GET" { // Jika user membuka halaman update, maka memulai method GET, menampilkan form update
			id := r.URL.Query().Get("id")
